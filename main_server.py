...
class MainServer:
    def __init__(self):
        xml_string = eau.get_database_from_url('http://openapi.forest.go.kr/openapi/service/cultureInfoService/gdTrailInfoOpenAPI?serviceKey=yn8uwUR3eheqowtPnA9QRTQ9i8mYGhGEetp6HDG1hMhCeH9%2BNJFN6WlIM1AzfgrZB59syoKUT1rAVveE9J6Okg%3D%3D&searchMtNm=&searchArNm=&pageNo=1&numOfRows=100')
        raw_data = eau.xml_to_json(xml_string)
        mntnm, aeatreason, overview, details = eau.get_useful_data(raw_data)
        self.server_socket, self.socks = eau.socket_initialize('10.10.21.121', 9000)
        self.turn_server_on()
        ...
        while True:
            ...
            for sock in read_socket:
                if sock == self.server_socket:
                    client_socket, addr, self.socks = eau.add_client_to_socket_list(sock, self.socks)
                    ...
                    try:
                        ...
                        if not data:
                            self.socks = eau.connection_lost(sock, self.socks)
                            continue
    ...
    def check_answer(self, answer, client_sock):
        ...
        correct_answer = eau.execute_db(sql)[0][0]
